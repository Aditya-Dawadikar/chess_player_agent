...
def aggregate_data(directory: str):
    ...
    for outcome in all_results:
        if outcome["winner"] == 'W':
            ...
            avg_captures_per_turn_p1.append(outcome["p1_capture_count"] /
            ...
            avg_captures_per_turn_p2.append(outcome["p2_capture_count"] /
...
def visualize_data(aggregate: dict):
    ...
    axs[0, 1].plot(p1_points_per_match,
                   label='P1 Points per Match',
                   marker='o',
                   color='blue')
    axs[0, 1].plot(p2_points_per_match,
                   label='P2 Points per Match',
                   marker='x',
                   color='red')
    axs[0, 1].axhline(np.mean(p1_points_per_match),
                      color='blue',
                      linestyle='--',
                      label='P1 Avg Points')
    axs[0, 1].axhline(np.mean(p2_points_per_match),
                      color='red',
                      linestyle='--',
                      label='P2 Avg Points')
    ...
    axs[0, 2].plot(p1_captures_per_match,
                   label='P1 Captures per Match',
                   marker='o',
                   color='green')
    axs[0, 2].plot(p2_captures_per_match,
                   label='P2 Captures per Match',
                   marker='x',
                   color='orange')
    axs[0, 2].axhline(np.mean(p1_captures_per_match),
                      color='green',
                      linestyle='--',
                      label='P1 Avg Captures')
    axs[0, 2].axhline(np.mean(p2_captures_per_match),
                      color='orange',
                      linestyle='--',
                      label='P2 Avg Captures')
    axs[0, 2].set_title("Captures per Match")
    axs[0, 2].legend()

    # Plot Total Points per Player
    axs[1, 0].bar(['P1 Total Points', 'P2 Total Points'],
                  [p1_points, p2_points],
                  color=['green', 'red'])
    ...
    axs[1, 1].bar(['P1 Avg Captures/Match', 'P2 Avg Captures/Match'],
                  [avg_captures_per_match_p1, avg_captures_per_match_p2],
                  color=['blue', 'orange'])
    ...
    axs[1, 2].bar(['P1 Avg Points/Match', 'P2 Avg Points/Match'],
                  [avg_points_per_match_p1, avg_points_per_match_p2],
                  color=['green', 'red'])
